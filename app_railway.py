...
from config_railway import settings
...
async def _process_questions_internal(request: QuestionRequest):
    try:
        ...
        if not pdf_content.strip():
            return QuestionResponse(
                answers=["Could not extract text from the provided document."],
                processing_time=0
            )
        
        # Create vector store
        await asyncio.wait_for(
            vector_store.add_documents([pdf_content]),
        ...
        return QuestionResponse(answers=answers, processing_time=0)
...
@app.get("/")
async def root():
    return {"message": "HackRx AI QA System is running on Railway!"}
